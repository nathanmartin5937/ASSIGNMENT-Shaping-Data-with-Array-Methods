...
const fileBuilder = () => {
    ...
    businesses.forEach((item, i) => {
      domString += `<div id="${i}" class="card">
              <div class="card-body">
              <p class="cardText">${item.purchasingAgent}</p> 
              <p class="cardText">${item.phoneWork}</p> 
              <p class="cardText">${item.orders}</p> 
              <p class="cardText">${item.companyName}</p> 
              <p class="cardText">${item.companyIndustry}</p> 
              <p class="cardText">${item.addressZipCode}</p> 
              <p class="cardText">${item.addressStateCode}</p> 
              <p class="cardText">${item.addressFullStreet}</p> 
              <p class="cardText">${item.addressCity}</p>       
              </div>
            </div>`
    });
...
const printToDom = (divId, textToPrint) => {
    ...
    const pullForm = (e) => {
        ...
        const agent = document.querySelector('#purchasingAgent').value;
        const phone = document.querySelector('#phoneWork').value;
        const orders = document.querySelector('#orders').value;
        const company = document.querySelector('#companyName}').value;
        const industry = document.querySelector('#companyIndustry').value;
        const zip = document.querySelector('#addressZipCode').value;
        const state = document.querySelector('#addressStateCode').value;
        const street = document.querySelector('#addressFullStreet').value;
        ...
        businesses.push(objects);
        pinBuilder(businesses);
